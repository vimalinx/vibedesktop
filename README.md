...
> Desktop, including commercially. See [License](#license).
...
- if `vibed` is not already on `PATH`, pipes the existing [vibed installer](#local-webapps-via-vibed) through `sh` first — the two installers compose, they do not duplicate;
...
State (the desktop JSON and `vibed` runtime state) lives under `${XDG_DATA_HOME:-~/.local/share}/vibedesktop-data/` and is never touched by an update or uninstall unless you pass `--purge-data --confirm-purge-data`.
...
Every browser-side read and write of desktop state goes through one client
module (`src/lib/desktop-data/`), which has two implementations chosen by
`NEXT_PUBLIC_VIBE_DATA_MODE`:

| Mode | State lives in | Server-dependent features |
| --- | --- | --- |
| `server` (default) | the local Next server's JSON file — the real product | all available |
| `local` | the visitor's own browser storage | absent, and labelled as such |

`local` exists for a read-only-feeling online trial: no server means no shared
state, so visitors cannot overwrite one another's desktop. Anything that needs a
machine to act on — reading a site's title and icon, checking whether a URL
allows framing, and the whole Local WebApps / `vibed` family — is **absent**
rather than faked, because those operations run server-side precisely so the
browser never fetches arbitrary URLs itself. The panels that need them say so.

Everything else behaves identically in both modes, because both call the same
desktop rules in `src/lib/desktop-core.ts` — seeding, grid placement, tile
validation. The trial's app store reads the catalog from `public/catalog.json`:

```bash
npm run catalog:build && npm run catalog:stage   # dist/catalog.json → public/catalog.json
```

## Cutting a release

Distribution is static — no server required. The complete release qualification stages the public allowlist and performs signed, isolated daemon and whole-app installations:
