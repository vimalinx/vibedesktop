...
The release tag must exactly equal `v` plus the version in `package.json`. For example, package version `0.1.4` uses tag `v0.1.4`:

```bash
git tag -a v0.1.4 -m "Vibe Desktop v0.1.4"
