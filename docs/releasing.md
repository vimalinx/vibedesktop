...
The release tag must exactly equal `v` plus the version in `package.json`. For example, package version `0.1.2` uses tag `v0.1.2`:

```bash
git tag -s v0.1.2 -m "Vibe Desktop v0.1.2"
