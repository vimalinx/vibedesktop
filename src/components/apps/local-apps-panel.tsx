...
import { TrialUnavailablePanel } from "@/components/chrome/trial-unavailable";
...
export function LocalAppsPanel({
  ...
  async function registerCandidate(candidate: LocalAppDiscoveryCandidate) {
    if (!localApps || !candidate.command || !candidate.registerable || candidate.alreadyRegistered) return;
  ...
  if (!localApps) {
    return <TrialUnavailablePanel t={messages} body={messages.trial.localAppsBody} />;
  }
