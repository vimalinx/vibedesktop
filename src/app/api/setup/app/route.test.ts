...
describe("GET /api/setup/app", () => {
  ...
  it("exposes the documented CLI subcommands and the systemd unit name", () => {
    ...
    // Composition with vibed must use the canonical user-scoped CLI, avoiding
    // an unrelated executable with the same name elsewhere on PATH.
    expect(installer).toContain('VIBED_BIN="$HOME/.local/bin/vibed"');
    expect(installer).toContain('if [ ! -x "$VIBED_BIN" ]');
