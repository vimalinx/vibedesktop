...
describe("GET /api/setup/app", () => {
  ...
  it("exposes the documented CLI subcommands and the systemd unit name", () => {
    ...
    // Composition with vibed must be present but gated behind `command -v vibed`.
    expect(installer).toContain("command -v vibed");
  ...
  it.runIf(process.env.VIBE_TEST_APP_INSTALLER_E2E === "1")(
    ...
    async () => {
      ...
      expect(unitText).toContain("VIBE_DAEMON_DATA_DIR=%h/.local/share/vibedesktop-data/vibed-vibedesktop");
