...
describe("restoredWindowDragOrigin", () => {
  it("keeps the grabbed horizontal titlebar point under the pointer", () => {
    expect(
      restoredWindowDragOrigin({
        pointerX: 840,
        pointerY: 120,
        horizontalGrabRatio: 0.5,
        verticalGrabOffset: 18,
        restoredWidth: 780,
        viewportWidth: 1400
      })
    ).toEqual({ x: 450, y: 102 });
  ...
  it("keeps a restored titlebar reachable at both viewport edges", () => {
    expect(
      restoredWindowDragOrigin({
        pointerX: 20,
        pointerY: 16,
        horizontalGrabRatio: 0.1,
        verticalGrabOffset: 14,
        restoredWidth: 780,
        viewportWidth: 1400
      })
    ).toEqual({ x: 12, y: 12 });
    ...
    expect(
      restoredWindowDragOrigin({
        pointerX: 1390,
        pointerY: 96,
        horizontalGrabRatio: 0.9,
        verticalGrabOffset: 16,
        restoredWidth: 780,
        viewportWidth: 1400
      })
    ).toEqual({ x: 608, y: 80 });
  ...
  it("keeps an oversized saved window anchored to the safe inset", () => {
    expect(
      restoredWindowDragOrigin({
        pointerX: 500,
        pointerY: 80,
        horizontalGrabRatio: 0.5,
        verticalGrabOffset: 20,
        restoredWidth: 1100,
        viewportWidth: 900
      })
    ).toEqual({ x: 12, y: 60 });
...
describe("window sizing + grid", () => {
  ...
  it("initialWindowSize keeps Local WebApps compact and distinct from the system monitor", () => {
    const localApps = urlApp({
      kind: "builtin",
      title: "Local WebApps",
      iconUrl: "vd://icon/local-apps",
      url: null
    });
    const startBoard = urlApp({
      kind: "builtin",
      title: "Start Board",
      iconUrl: "vd://icon/start-board",
      url: null
    });
    expect(initialWindowSize(localApps)).toEqual({ width: 780, height: 440 });
    expect(initialWindowSize(localApps).height).toBeLessThan(initialWindowSize(startBoard).height);
  });
