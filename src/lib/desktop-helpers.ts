...
export const windowEdgeInset = 12;

export interface RestoredWindowDragOriginInput {
  pointerX: number;
  pointerY: number;
...
/**
 * Keep the same point of a maximized titlebar under the pointer when the
 * window is restored during a drag. The edge clamp guarantees that some
 * titlebar always remains reachable, even when the saved window is wider than
 * the current viewport.
 */
export function restoredWindowDragOrigin({
  pointerX,
  pointerY,
  horizontalGrabRatio,
  verticalGrabOffset,
  restoredWidth,
  viewportWidth
}: RestoredWindowDragOriginInput): { x: number; y: number } {
  const grabRatio = Math.min(1, Math.max(0, horizontalGrabRatio));
  const unclampedX = pointerX - restoredWidth * grabRatio;
  const maximumX = Math.max(windowEdgeInset, viewportWidth - restoredWidth - windowEdgeInset);

  return {
    x: Math.min(maximumX, Math.max(windowEdgeInset, unclampedX)),
    y: Math.max(windowEdgeInset, pointerY - verticalGrabOffset)
  };
}
...
export function initialWindowSize(app: DesktopApp): { width: number; height: number } {
  ...
  if (isLocalAppsApp(app)) return { width: 780, height: 440 };
