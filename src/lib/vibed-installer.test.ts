...
import { VIBE_RELEASE_VERSION } from "@/lib/vibed-release-manifest";
import {
...
import { BUNDLED_NODE_RELEASES, BUNDLED_NODE_VERSION as DAEMON_BUNDLED_NODE_VERSION } from "../../daemon/release/bundled-node.mjs";

const requiredRuntimePaths = [
  "daemon/server.mjs",
  "daemon/cli-entry.mjs",
  "daemon/release/version-store.mjs",
  "daemon/release/lifecycle-update.mjs",
  "daemon/release/lifecycle-uninstall.mjs"
];
...
describe("vibed installer", () => {
  ...
  it("stays in sync with daemon/release/bundled-node.mjs's pinned version and checksums", () => {
  ...
  it("throws when a required runtime file is missing", () => {
    ...
    files.runtime = files.runtime.filter((file) => file.path !== "daemon/release/version-store.mjs");
  ...
  it("produces a deterministic version id for identical files and a different one when content changes", () => {
    ...
    expect(versionIdA).toBe(`${VIBE_RELEASE_VERSION}+${versionIdA.split("+")[1]}`);
    expect(versionIdA.split("+")[1]).toMatch(/^[0-9a-f]{12}$/);
