...
export function buildAppInstaller(): string {
  const wrapperTemplate = [
    ...
    'export VIBE_DAEMON_DATA_DIR="$DATA_DIR/vibed-vibedesktop"',
  ...
  const serviceTemplate = [
    ...
    "[Service]",
    "Type=simple",
    "WorkingDirectory=%h/.local/share/vibedesktop/app/current",
    "ExecStart=%h/.local/bin/vibedesktop-run",
    "Restart=on-failure",
    "RestartSec=3",
    "TimeoutStopSec=15",
    "Environment=NODE_ENV=production",
    "Environment=VIBE_DATA_FILE=%h/.local/share/vibedesktop-data/vibedesktop.json",
    "Environment=VIBE_DAEMON_DATA_DIR=%h/.local/share/vibedesktop-data/vibed-vibedesktop",
...
# Compose with vibed: if the local WebApp manager is not installed, install it
# first so the app can manage local servers on first boot. --install-only keeps
# the systemd enable/restart below owned by this installer alone.
if ! command -v vibed >/dev/null 2>&1; then
  printf '%s\\n' 'Installing the vibed daemon (local WebApp manager)...'
  if command -v curl >/dev/null 2>&1; then
    curl -fsSL "$VIBED_INSTALLER_URL" | sh -s -- --install-only
  elif command -v wget >/dev/null 2>&1; then
    wget -qO- "$VIBED_INSTALLER_URL" | sh -s -- --install-only
  else
    printf '%s\\n' 'vibed is not installed and curl/wget are unavailable to fetch it.' >&2
    exit 1
  fi
fi
...
while [ "$ATTEMPT" -lt 40 ]; do
  if curl -fsS "http://127.0.0.1:$APP_PORT/api/desktop" >/dev/null 2>&1; then
