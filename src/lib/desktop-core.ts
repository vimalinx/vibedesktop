 ...
 * else. `store.ts` owns reading and writing the local JSON file; browser and API
 * behavior both call these same rules so there is only one product behavior to
 * maintain.
 *
 * Nothing here may import a `node:` module. The only ambient calls used are
 * `crypto.randomUUID()` and `new Date()`.
 */
