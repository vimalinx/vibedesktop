...
export class ConfigStore {
  ...
  async load() {
    try {
      ...
      return parsed.apps.filter(isValidConfig).map(projectStoredConfig);
    } catch (err) {
...
/**
 * Project records read from disk onto the public LocalAppConfig contract.
 * Older versions could persist experimental fields; they must never leak back
 * through the local API or survive the next write.
 * @param {Record<string, unknown>} value
 * @returns {LocalAppConfig}
 */
function projectStoredConfig(value) {
  const projected = /** @type {LocalAppConfig} */ ({
    id: value.id,
  ...
  if (Array.isArray(value.args)) projected.args = value.args.filter((item) => typeof item === "string" && item.length > 0);
  if (typeof value.cwd === "string" && value.cwd) projected.cwd = value.cwd;
  if (value.env && typeof value.env === "object" && !Array.isArray(value.env)) {
    projected.env = /** @type {Record<string, string>} */ (value.env);
  }
  if (typeof value.autoStart === "boolean") projected.autoStart = value.autoStart;
  if (value.restart === "no" || value.restart === "on-crash" || value.restart === "always") projected.restart = value.restart;
  if (value.iconKind === "favicon" || value.iconKind === "custom_local" || value.iconKind === "fallback") projected.iconKind = value.iconKind;
  if (typeof value.iconUrl === "string" || value.iconUrl === null) projected.iconUrl = value.iconUrl;
