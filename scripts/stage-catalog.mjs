 ...
 *
 * The browser-local data source (the online trial) has no API route to ask, so
 * it reads the artifact straight off the origin. This is a copy step, not a
 * second generator — `npm run catalog:build` remains the only thing that
 * produces a catalog, so the trial and the server can never be looking at two
